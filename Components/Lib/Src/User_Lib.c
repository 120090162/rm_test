 ...
 * @return    转换后的角度
 */
float theta_transform(float angle, float dangle, int8_t direction, uint8_t duration)
{
    return loop_float_constrain((angle + dangle) * direction, -PI * duration, PI * duration);
}
